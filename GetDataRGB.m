%% **** Used to extract faces from Images and Videos for CNN **** %%
%%  Clear workspace 
clc; clear all; close all;

%% Root Directory
rootPath = '/Users/vithurshan/Desktop/Data/';
newPath = '/Users/vithurshan/Desktop/FinalRGB/';
...
%list = [3, 5, 8,9, 12, 14,15, 18, 20,21, 24, 26,27, 30, 33, 35,36, 39,42, 44]; 
    ...
    for i = 1:size(videos,1)
        ...
        for frame = 1: numberOfFrames
            if ismember(frame, list)
                ...
                if maxValue > 1
                    %get bunding box using face detection 
                    boundingBox = faceDetection(thisFrame);
                    if isempty(boundingBox)
                       boundingBox = frontFaceDetection(thisFrame);
                        if isempty(boundingBox)
                            boundingBox = profileFaceDetection(thisFrame);
                            if isempty(boundingBox)
                                continue%
                            elseif size(boundingBox,1)> 1
                                [val, idx] = max(boundingBox(:,3));
                    ...
                    %crop image
                    croppedFace = imcrop(thisFrame,boundingBox);
                    %resize image
                    resizeImage = imresize(croppedFace, scale);
